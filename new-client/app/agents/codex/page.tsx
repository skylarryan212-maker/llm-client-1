...
type TabKey = "tasks" | "reviews" | "archive";
...
export default function CodexAgentPage() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("agents-codex-tasks");
    if (!stored) return;

    try {
      setTasks(JSON.parse(stored));
    } catch (error) {
      console.error("Failed to parse stored Codex tasks", error);
    }
  }, []);

  useEffect(() => {
    localStorage.setItem("agents-codex-tasks", JSON.stringify(tasks));
  }, [tasks]);
  ...
  const handleSubmit = (message: string) => {
    ...
    router.push(`/agents/codex/c/${newChatId}?prompt=${encodeURIComponent(message)}`);
  };
          ...
          <div className="w-full max-w-2xl">
            {activeTab === "tasks" && (
              <div className="space-y-4">
                ...
                {tasks.length > 0 ? (
                  tasks.map((task) => <CodexTaskItem key={task.id} {...task} />)
                ) : (
                  <p className="text-center text-muted-foreground py-8">
                    No tasks yet. Create one with the composer above.
                  </p>
                )}
              </div>
            )}
