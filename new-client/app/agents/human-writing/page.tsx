...
export default function HumanWritingAgentPage() {
  return (
    <Suspense fallback={<div />}>
      <HumanWritingAgentPageInner />
    </Suspense>
  );
}

function HumanWritingAgentPageInner() {
  ...
  // Restore chat mode if URL already has chat param
  useEffect(() => {
    const existing = searchParams.get("chat");
    if (existing && !chatStarted) {
      setChatStarted(true);
      setChatId(existing);
      if (!messages.length) {
        setMessages([{ id: "assistant-welcome", role: "assistant", content: "Coming soon - chat session restored." }]);
      }
    }
  }, [chatStarted, messages.length, searchParams]);

  const handleSend = () => {
    const trimmed = composerText.trim();
    if (!trimmed) return;
    const id = chatId ?? `hw-${Date.now()}`;
    setChatId(id);
    setChatStarted(true);
    router.replace(`/agents/human-writing?chat=${id}`, { scroll: false });
    setMessages((prev) => [
      ...prev,
      { id: `u-${Date.now()}`, role: "user", content: trimmed },
      { id: `a-${Date.now()}`, role: "assistant", content: "Coming soon - full pipeline wiring is in progress." },
    ]);
    setComposerText("");
  };

  const composerShellClasses = chatStarted ? "max-w-[880px]" : "max-w-[960px]";
  const composerHeightClasses = chatStarted ? "min-h-[120px] pb-16" : "min-h-[200px] pb-28";
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#0f0d12] text-foreground">
      ...
      {chatStarted && (
        <div className="mx-auto flex w-full max-w-[960px] flex-col gap-4 px-4 pb-32 sm:px-6 lg:px-8">
          <div className="rounded-[18px] border border-white/10 bg-[#111118]/80 p-4 shadow-lg ring-1 ring-white/5">
            <div className="space-y-3">
              {messages.map((msg) => (
                <div key={msg.id} className="rounded-2xl bg-white/5 px-4 py-3 text-sm text-white/90">
                  <div className="mb-1 text-[11px] uppercase tracking-[0.25em] text-white/40">
                    {msg.role === "user" ? "You" : "Assistant"}
                  </div>
                  <p className="leading-relaxed text-white/90">{msg.content}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-0 flex justify-center pb-6">
        <div
          className={`pointer-events-auto relative w-full ${composerShellClasses} rounded-[22px] border border-white/20 bg-[#121217]/90 p-6 shadow-[0_40px_80px_rgba(0,0,0,0.6)] backdrop-blur-3xl transition-all duration-300`}
        >
          <Textarea
            value={composerText}
            onChange={(event) => setComposerText(event.target.value)}
            placeholder="Describe the essay or writing task..."
            className={`resize-none bg-transparent !bg-transparent text-white placeholder:text-white/50 shadow-none outline-none ring-0 border-0 focus-visible:ring-0 focus-visible:outline-none focus-visible:border-0 focus-visible:shadow-none pr-32 transition-[min-height,padding] duration-300 ${composerHeightClasses}`}
          />
